...
class MNN:
    def __init__(self,
        ...
        self._predict_op = tf.argmax(self._output[:, 0, :], axis = 1, name = "predict_op") # Index of answer word
    ...
    def _build_training(self):
        # Only use first word of the RNN's output sentence
        last_words = self._output[:, -1, :]

        # Cross entropy loss
        cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits = last_words, labels = tf.cast(self._answers, tf.float32), name = "cross_entropy")
    ...
    def _train_batches_SGD(self, data, learning_rate):
        # Prepare batches
        num_batches = int(math.ceil(len(data) / self._batch_size))
        batch_indices = list(range(0, len(data) - self._batch_size, self._batch_size))
        ...
        for batch_index in batch_indices:
            # Get batch
            batch = data[batch_index : batch_index + self._batch_size]
    ...
    def accuracy(self, data):
        """ Computes the model's accuracy, defined as the proportion of correct predictions on the given data, without training it.
    ...
    def predict(self, sentence_context, queries):
        ...
        @param sentence_context: (np.array) Numpy array with shape (memory_size, max_sentence_length) representing a list of context sentences.
        ...
        with self._sess.as_default():
            ...
            prediction = self._sess.run(self._predict_op, feed_dict = feed_dict)

        return prediction

    def predict_word(self, sentences, query):
        """ Given a list of sentences and a query, returns the model's single-word answer.
        
        @param sentences: (list(str)) List of raw sentences.
        @param query: (str) Raw query sentence.
        ...
        prediction = self.predict(sentence_context, queries)

        # Return raw word
        return self._index_to_word[prediction]
